...
import com.prodain.scf.common.repo.EntityGraphOffsetLimitQuerydslPredicateExecutor;
import com.prodain.scf.common.repo.OffsetLimit;
import com.prodain.scf.common.repo.OffsetLimitQuerydslPredicateExecutor;
...
public abstract class AbstractJpaRestController
{
	...
	protected <T> ResponseEntity<?> createResponse(Iterable<T> list, RestRequest restRequest, Class<T> resourceClass, Function<T, ResponseWrapper<T>> wrapperFunction)
	{
		...
		if(list instanceof Page)
		{
			...
			if(wrapperFunction != null) {
				...
				return response.body(restEngine.applyJsonFilter(toPageResponse(page,wrapped), resourceClass, restRequest));
			}else {
				return response.body(restEngine.applyJsonFilter(toPageResponse(page), resourceClass, restRequest));
			}
		}
	...
	protected <T> PageResponse toPageResponse(Page<T> list) {
		return toPageResponse(list, null);
	}
	
	protected <T> PageResponse toPageResponse(Page<T> list, List<ResponseWrapper<T>> wrapped) {
		PageResponse pageResponse = new PageResponse();
		pageResponse.setCount((int) list.getTotalElements());
		pageResponse.setPageSize(list.getSize());
		pageResponse.setPage(list.getNumber()+1);
		pageResponse.setPageCount(list.getTotalPages());
		if(wrapped != null)
			pageResponse.setResults(wrapped);
		else
			pageResponse.setResults(list.getContent());
		return pageResponse;
	}
